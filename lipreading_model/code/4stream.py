...
def train(device, model, optimizer, datagen, epoch, epochsize, \
          s2_train_X, s3_train_X, s4_train_X, \
...
#         print(X.shape,X_s2.shape)
        output,ordered_idx = model(X, X_s2, X_s3, X_s4,  vid_lens_batch)
...
def test(device, model,name,epoch, X_s1_val,X_s2_val, X_s3_val, X_s4_val, y_val, \
    ...
    output,ordered_idx = model(X1, X2, X3, X4, vid_lens_batch)
...
def main():
    ...
    parser.add_argument('--four_stream_views', type=str, default="1,2,3,4", help='4stream views \
    to train like 1,2,3,4 ; 1,2,3,5 ; 1,2,4,5 and so on. Please seperate by comma. \
    1 --> 0deg, 2 --> 30deg, 3 --> 45deg, 4 --> 60deg, 5 --> 90deg')

    parser.add_argument('--save_path', type=str, default='../results/4stream', help='path where \
...
#     num_epoch= 40
    ...
    network = adenet_4stream(device, pretrained_stream1_model_isTrue, \
    ...
    for epoch in range(1,num_epoch+1):
        ...
        #(device, model, optimizer, datagen, epoch, epochsize, s2_train_X, s3_train_X, s4_train_X,\
    #     s1_train_vidlens, integral_lens)
        train(device, network, optimizer, datagen, epoch, epochsize, s2_train_X, s3_train_X, s4_train_X,\
            s1_train_vidlens, integral_lens)
    ...
    #   train_acc, train_loss,  predictions_train, true_label_train = test(device, model,"train",epoch, X_train, y_train,  vid_lens_batch_train, mask_train, idxs_train)
        ...
        #(device, model,name,epoch, X_s1_val,X_s2_val, X_s3_val, X_s4_val, y_val, vid_lens_batch, mask_val, idxs_val)
        val_acc, val_loss, predictions_val, true_label_val = test(device, model,"val",epoch, \
                                    X_s1_val,X_s2_val, X_s3_val, X_s4_val, \
        ...
        test_acc, test_loss, predictions_test, true_label_test = test(device, model,"test",epoch, \
                                X_s1_test,X_s2_test, X_s3_test, X_s4_test, \
